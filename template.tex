...
I describe two aspects of network virtualization that can benefit from the use of shared-logs 

possible extensions to the SDN framework that could benefit the application of network virtualization.

The network state of SDN when network virtualization cannot 
...
Provide network management view abstractions.
...
network information base has the topology graph on each controller
typed entries

accessing network things
no locking, but request access on local instance only
    external locking coordination later

all async, eventual guaranteez
syncronization primative, an implement cooridnation if ya want so no race cond

scalability n reliability
partition so controller has some of nib in memory
    few switches, but all routing state exceeds mem of any server
    replicate all switch state, but partition forwarding state and assign to diff ornix instances
    keep switch and link state in replicated durable inventory
    link utilization in DHT
        like MPLS?

    can also break down NIBs
aggregate as single node in hierarchy 
    avoid remote instances having high-rates of updates 
    squish subtopology into single logical node
    like ATM PNNI
    still have full connectivity

    aggregate interdomain (not done here)

reliability
    network link failure
        steer traffic around
    onix fail
        scan and deal with using coordination
...
distribute nib
    ...
    return openflow events and stuff
