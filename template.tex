...
Imagine a network configuration with two groups that must be kept separate---Imagine that North and South Korea are on the same SDN network, and we want no packets to flow between the North Korea Virtual Network Segement and the South Korea VNS. In this case, 

In particular, this could be import for effcective separation of regions in network virtualization situations.
